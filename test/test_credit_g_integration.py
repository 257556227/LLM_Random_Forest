...
class TestCreditGIntegration(unittest.TestCase):
    def setUp(self):
        self.root = Path(__file__).resolve().parents[1]
        self.delta_root = self.root / "DeLTa-main"

    def test_credit_g_data_and_artifacts_exist(self):
        dataset_dir = self.delta_root / "example_datasets" / "credit-g"
        results_dir = self.delta_root / "results" / "credit-g"
        readiness_file = results_dir / "credit-g_integration_readiness.json"
        ...
        same_metric_file = results_dir / "credit_g_leaf_expansion_same_metric_summary.json"
        formal_protocol_file = results_dir / "credit_g_leaf_expansion_formal_protocol_summary.json"
