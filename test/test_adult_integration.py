...
class TestAdultIntegration(unittest.TestCase):
    def setUp(self):
        self.root = Path(__file__).resolve().parents[1]
        self.delta_root = self.root / "DeLTa-main"

    def test_adult_data_and_artifacts_exist(self):
        dataset_dir = self.delta_root / "example_datasets" / "adult"
        results_dir = self.delta_root / "results" / "adult"
        readiness_file = results_dir / "adult_integration_readiness.json"
        ...
        same_metric_file = results_dir / "adult_leaf_expansion_same_metric_summary.json"
        formal_protocol_file = results_dir / "adult_leaf_expansion_formal_protocol_summary.json"
        ...
        self.assertEqual(summary["dataset"], "adult")
        self.assertEqual(formal_protocol["dataset"], "adult")
