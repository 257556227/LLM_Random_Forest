...
def _get_category_encoders_module():
    try:
        import category_encoders
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "category_encoders is required only for binary/hash/loo/target/catboost categorical policies."
        ) from exc
    return category_encoders
...
def data_enc_process(N_data, C_data, cat_policy, y_train = None, ord_encoder = None, mode_values = None, cat_encoder = None):
    ...
    if C_data is not None:
        ...
        # use other encoding if we will treat categorical features as numerical
            ...
            category_encoders = _get_category_encoders_module()
            if cat_encoder is None:
...
# def data_enc_process_2(N_data, C_data, cat_policy, y_train = None, ord_encoder = None, mode_values = None, cat_encoder = None):
#     """
#     Process the categorical features in the dataset.
...
#     :return: Tuple[ArrayDict, ArrayDict, Optional[OrdinalEncoder], Optional[List[int]], Optional[OneHotEncoder]]
#     """
#     print('data_enc_process_2_cat_policy',cat_policy)
...
def data_label_process(y_data, is_regression, info = None, encoder = None):
    ...
    if is_regression:
        ...
        if info is None:
            ...
            ymax = np.max(y_data['train'])
            ymin = np.min(y_data['train'])
            ymax_minus_ymin = ymax - ymin
        else:
