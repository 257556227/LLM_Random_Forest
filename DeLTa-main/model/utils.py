...
def _get_tabpfn_regressor():
    try:
        from tabpfn import TabPFNRegressor
    except ModuleNotFoundError as exc:
        raise ModuleNotFoundError(
            "tabpfn is required only when small_model=tabpfn; current environment does not provide it."
        ) from exc
...
def resolve_small_model_for_runtime(small_model: str) -> str:
    if small_model == 'tabpfn' and not torch.cuda.is_available():
        return 'cart'
    return small_model
    ...
    if not is_regression:
        ...
        for leaf_id, indices in leaf_index_dict.items():
            ...
            if args.small_model == 'cart':
                ...
                TabPFNRegressor = _get_tabpfn_regressor()
                if set_.shape[0] > 1 and leaf_id not in one_model_list.keys():
                    model = TabPFNRegressor(
                        device=get_tabpfn_device(),
