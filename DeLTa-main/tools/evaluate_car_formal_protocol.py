...
def _load_dataset_loader():
    ...
    spec = importlib.util.spec_from_file_location("car_leaf_expansion_runner", module_path)
...
RESULTS_DIR = PROJECT_ROOT / "results" / "car"
...
MAINLINE_RF_FILE = RESULTS_DIR / "car_md10_ml10_tree20.npy"
MAINLINE_DELTA_LOG = RESULTS_DIR / "e_RF_md10_ml10_tree20_car_cart_0.log"
OUTPUT_FILE = RESULTS_DIR / "car_leaf_expansion_formal_protocol_summary.json"
...
def _safe_multiclass_auc(labels: np.ndarray, scores: np.ndarray) -> float:
...
def _load_mainline_rf_result(file_path: Path) -> dict[str, Any]:
    ...
    logits = np.asarray(payload["logit"], dtype=float)
    ...
    return {
        ...
        "test_auc": _safe_multiclass_auc(labels, logits),
    }


def _parse_metric(log_text: str, metric_name: str) -> float:
...
def _load_mainline_delta_result(log_file: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "sample_count": None,
        "test_accuracy": _parse_metric(log_text, "Fused Accuracy MEAN"),
        "test_auc": _parse_metric(log_text, "Fused AUC MEAN"),
        "eta": _parse_metric(log_text, "Fusion eta"),
    }
...
def _build_deeper_rf(X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray) -> dict[str, Any]:
    ...
    return {
        "source_mode": "car depth=4 random_forest same-metric recompute",
        "sample_count": int(y_test.shape[0]),
        "test_accuracy": float(accuracy_score(y_test, predictions)),
        "test_auc": _safe_multiclass_auc(y_test, probabilities),
        "n_estimators": 100,
        "max_depth": 4,
    }
...
def build_summary() -> dict[str, Any]:
    ...
    feature_parts, target_parts, _info = load_dataset("car", "example_datasets")
    ...
    return {
        "protocol_name": "car_leaf_expansion_formal_protocol_v1",
        "dataset": "car",
        ...
        "notes": [
            "这份摘要把 car 的 leaf expansion 原型、depth=4 Deeper RF，以及传统 DeLTa/RF 主链结果放到同一张协议表里。",
            "car 是低维纯类别多分类数据，因此这份正式协议特别适合沉淀类别值解释、target-conditioned prompt 与候选叶子策略的通用化经验。",
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"Missing required input files for car formal protocol summary: {missing}")

    OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    summary = build_summary()
    OUTPUT_FILE.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Wrote car formal protocol summary to {OUTPUT_FILE}")
