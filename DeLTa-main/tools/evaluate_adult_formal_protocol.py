...
def _load_dataset_loader():
    ...
    spec = importlib.util.spec_from_file_location("adult_leaf_expansion_runner", module_path)
...
MAINLINE_RF_FILE = RESULTS_DIR / "adult_md20_ml50_tree15.npy"
MAINLINE_DELTA_LOG = RESULTS_DIR / "e_RF_md20_ml50_tree15_adult_cart_0.log"
OUTPUT_FILE = RESULTS_DIR / "adult_leaf_expansion_formal_protocol_summary.json"
...
def _build_deeper_rf(X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray) -> dict[str, Any]:
    ...
    return {
        "source_mode": "adult depth=4 random_forest same-metric recompute",
...
def build_summary() -> dict[str, Any]:
    ...
    feature_parts, target_parts, _info = load_dataset("adult", "example_datasets")
    ...
    return {
        "protocol_name": "adult_leaf_expansion_formal_protocol_v1",
        "dataset": "adult",
        ...
        "notes": [
            "这份摘要把 adult 的 leaf expansion 原型、depth=4 Deeper RF，以及传统 DeLTa/RF 主链结果放到同一张协议表里。",
            "deeper_rf 使用相同 adult 数据划分现场重算的 depth=4 RandomForestClassifier，用来对应统一协议中的 Deeper RF 主对照。",
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"Missing required input files for adult formal protocol summary: {missing}")

    OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    summary = build_summary()
    OUTPUT_FILE.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Wrote adult formal protocol summary to {OUTPUT_FILE}")
