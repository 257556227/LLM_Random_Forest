...
def _load_dataset_loader():
    ...
    spec = importlib.util.spec_from_file_location("jannis_leaf_expansion_runner", module_path)
...
MAINLINE_RF_FILE = RESULTS_DIR / "jannis_md20_ml50_tree20.npy"
MAINLINE_DELTA_LOG = RESULTS_DIR / "e_RF_md20_ml50_tree20_jannis_cart_0.log"
OUTPUT_FILE = RESULTS_DIR / "jannis_leaf_expansion_formal_protocol_summary.json"
...
def _build_deeper_rf(X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray) -> dict[str, Any]:
    ...
    return {
        "source_mode": "jannis depth=4 random_forest same-metric recompute",
...
def build_summary() -> dict[str, Any]:
    ...
    feature_parts, target_parts, _info = load_dataset("jannis", "example_datasets")
    ...
    return {
        "protocol_name": "jannis_leaf_expansion_formal_protocol_v1",
        "dataset": "jannis",
        ...
        "notes": [
            "这份摘要把 jannis 的 leaf expansion 原型、depth=4 Deeper RF，以及传统 DeLTa/RF 主链结果放到同一张协议表里。",
            "jannis 是高维弱语义多分类数据，因此当前更看重局部统计摘要和候选叶子策略是否足够稳。",
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"Missing required input files for jannis formal protocol summary: {missing}")

    OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    summary = build_summary()
    OUTPUT_FILE.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Wrote jannis formal protocol summary to {OUTPUT_FILE}")
