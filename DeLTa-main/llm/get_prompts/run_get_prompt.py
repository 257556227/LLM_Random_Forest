...
def start():
    ...
    # 可选：通过环境变量指定Prompt模板文件（例如 mnist_stable.py）
    prompt_template_env = os.environ.get('DELTA_PROMPT_TEMPLATE')
            ...
            for md in mds:
                for ml in mls:
                    ...
                    for i in range(len(shot_range)):
                        transform_script = os.path.join(current_dir, 'transform.py')
                        prompt_template_path = os.path.join(
                            current_dir,
                            prompt_template_env if prompt_template_env else f'{dataset}.py',
                        )
                        cmd = [
